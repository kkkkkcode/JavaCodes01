...
* 需求：人画圆，请针对这个需求进行面向对象设计
  ...
  * 应该把“画圆”这个方法放在哪个对象中？
  ...
  - 封装方法中的一个重要思想：对象代表什么，就得封装对应的数据，并提供数据对应的行为
  
  * 画圆这件事情，需要根据**半径**来画,半径是圆的属性
  * 所以人画圆，是圆自己根据半径画的
```java
public class circle{
      double radius;
...
```
* 人关门：门是人关的，还是自己关的？ **一定是自己关的**
* 人做的事情，只是给门一个作用力，然后门自己把自己关上
...
- 还有其他好处吗？
  - JDK-API文档.CHM
  - 如：字符串对象String 封装了大量属性和大量可以调用的方法，如getBytes(),length(),toUpperCase();
  - 如：Socket对象，代表网络连接，如果想网络连接，连接别人的电脑，此时必定有一个属性记录了连谁，也必然有一个相应的连接方法和发送数据的方法，都会有
  - Java进行网络编程：创建对象，调方法就可以完成我们想做的事情了
  
  - 极大地降低了我们的学习成本，可以少学，少记，或者说压根不用学，不用记对象有哪些方法，有需要时去找就行
